  ...
  const server = new ApolloServer({
    ...
    resolvers: {
      Query: {
        ...
        contest: (_, { id }: { id: string }) => ({ id, name: 'Contest numero uno' }),
        vote: (_, { id }: { id: string }) => ({ id, userId: '1' }),
        drawingContest: (_, { id }: { id: string }) => ({ id, userId: '1', contestId: '2' })
      },
      Mutation: {
        createUser: async (_, { name }: { name: string }) => await UserDB.create({ name })
      }
    }
  })
